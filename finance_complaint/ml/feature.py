...
from pyspark.spl.functions import col, abs
    ...
    def _fit(self, dataframe: DataFrame):
        ...
        for column, new_column in zip(input_columns, output_columns):
            freq = (dataframe.select(col(column).
                                alias(f'g_{column}')).
                                groupby(f'g_{column}').
                                count().withColumn(new_columns, col('count')))
            freq = freq.drop('count')
            logging.info(f"{column} has [{freq.count()}] unique category")
    ...
    def _transform(self, dataframe: DataFrame):
        ...
        for column in inputCols:
            ...
            dataframe = dataframe.withColumn(self.getOutputCols()[0], 
                                    abs(col(inputCols[1]).cast(LongType()) - col(inputCols[0]).cast(LongType())) /
                                    (self.second_within_day))

            return dataframe

class FrequencyImputer(Transformer, HasInputCols, HasOutputCols, 
                                    DefaultParamsReadable, DefaultParamsWritable):
